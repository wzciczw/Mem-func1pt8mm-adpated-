...
try:
    ...
    create_feature_extractor = get_graph_node_names = None
...
try:
    ...
    open_clip = None
    VisionTransformer = Transformer = ResidualAttentionBlock = TimmModel = object
...
class AdaLNLoRACLIPViT(nn.Module):
    def __init__(
        ...
        if open_clip is None:
            raise ImportError("open_clip is required for CLIP backbones.")
...
_DEFAULT_DINOV2_REPO = os.path.join(
    _BACKBONE_DIR,
    "third_party",
    "facebookresearch_dinov2_main",
)
_DINO_REPO = os.environ.get("ENCODER_DINOV2_REPO", _DEFAULT_DINOV2_REPO)
if os.path.isdir(_DINO_REPO) and _DINO_REPO not in sys.path:
    sys.path.insert(0, _DINO_REPO)
...
class AdaLNLoRADiNOv2ViT(nn.Module):
    def __init__(
        ...
        # Try bundled local repo first, then torch hub cache/network.
        local_repo = os.environ.get("ENCODER_DINOV2_REPO", _DEFAULT_DINOV2_REPO)
        try:
            ...
            try:
                ...
                logging.warning(f"Failed to load pretrained weights for {ver}: {e}")
                logging.warning("Continuing with randomly initialized model...")
                vision_model = torch.hub.load(
                    "facebookresearch/dinov2",
                    ver,
                    source='local',
